...
regionBinCheckPval <- function(resMergeAll, inputASE, controlASE=NULL, rbp=NULL, outDir, binMotifUnique=F){
  ...
  if (binMotifUnique==T){
    resMergeStat <- resMerge %>%
      dplyr::select(idMap, spliceType, regionBinLoc, dataType, RBP, motifId) %>% 
      tidyr::gather(type, RBPmotifId,-c(seq(4))) %>% 
      dplyr::select(-type) %>% 
      dplyr::mutate(binDup=paste0(RBPmotifId, idMap, regionBinLoc)) %>%
